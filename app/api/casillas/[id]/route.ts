...
export async function PUT(
  ...
  try {
    ...
    // Verificar si ya existe otra casilla con el mismo nombre en la misma sección
  ...
  try {
    ...
    if (existingCasilla.votos.length > 0) {
      return NextResponse.json(
        {
          error:
            "No se puede eliminar la casilla porque tiene votos asociados. Elimine primero los votos.",
        },
        { status: 400 }
      );
    }
